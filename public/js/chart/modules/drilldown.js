(function (f) {
    ...
    l(q.lang, {drillUpText: "◁ Regresar a {series.name}"});
    q.drilldown = {activeAxisLabelStyle: {cursor: "pointer", color: "#0d233a", fontWeight: "bold", textDecoration: "underline"}, activeDataLabelStyle: {cursor: "pointer", color: "#0d233a", fontWeight: "bold", textDecoration: "underline"}, animation: {duration: 500}, drillUpButton: {position: {align: "right", x: -10, y: 10}}};
    ...
    m.prototype.addSingleSeriesAsDrilldown = function (a, b) {
        var c = a.series, d = c.xAxis, g = c.yAxis, e;
        e = a.color || c.color;
        var i, f = [], j = [], k, o;
        ...
        (o = this.drilldownLevels[this.drilldownLevels.length - 1]) && o.levelNumber !==
                k && (o = void 0);
        b = l({color: e, _ddSeriesId: z++}, b);
        i = y(a, c.points);
        h(c.chart.series, function (a) {
            if (a.xAxis === d && !a.isDrilling)
                a.options._ddSeriesId = a.options._ddSeriesId || z++, a.options._colorIndex = a.userOptions._colorIndex, a.options._levelNumber = a.options._levelNumber || k, o ? (f = o.levelSeries, j = o.levelSeriesOptions) : (f.push(a), j.push(a.options))
        });
        e = {levelNumber: k, seriesOptions: c.options, levelSeriesOptions: j, levelSeries: f, shapeArgs: a.shapeArgs, bBox: a.graphic ? a.graphic.getBBox() : {}, color: e, lowerSeriesOptions: b,
        ...
        if (d)
            d.oldPos = d.pos, d.userMin = d.userMax = null, g.userMin = g.userMax = null;
        if (c.type === e.type)
            e.animate = e.animateDrilldown || t, e.options.animation = !0
    };
    m.prototype.applyDrilldown = function () {
        var a = this.drilldownLevels, b;
        if (a && a.length > 0)
            b = a[a.length - 1].levelNumber, h(this.drilldownLevels,
                    function (a) {
    ...
    m.prototype.getDrilldownBackText = function () {
        ...
        if (a && a.length > 0)
            return a = a[a.length - 1], a.series = a.seriesOptions, B(this.options.lang.drillUpText, a)
    };
    m.prototype.showDrillUpButton = function () {
        var a = this, b = this.getDrilldownBackText(), c = a.options.drilldown.drillUpButton, d, g;
        this.drillUpButton ? this.drillUpButton.attr({text: b}).align() :
                (g = (d = c.theme) && d.states, this.drillUpButton = this.renderer.button(b, null, null, function () {
                    a.drillUp()
                }, d, g && g.hover, g && g.select).attr({align: c.position.align, zIndex: 9}).add().align(c.position, !1, c.relativeTo || "plotBox"))
    };
    m.prototype.drillUp = function () {
        for (var a = this, b = a.drilldownLevels, c = b[b.length - 1].levelNumber, d = b.length, g = a.series, e, i, f, j, k = function (b) {
            ...
            c = c || a.addSeries(b, !1);
            if (c.type === f.type && c.animateDrillupTo)
            ...
            if (i = b[d], i.levelNumber === c) {
                ...
                if (j.type === f.type)
                    j.drilldownLevel = i, j.options.animation = a.options.drilldown.animation, f.animateDrillupFrom && f.chart && f.animateDrillupFrom(i);
    ...
    n.prototype.animateDrilldown = function (a) {
        var b = this, c = this.chart.drilldownLevels, d, g = this.chart.options.drilldown.animation, e = this.xAxis;
        if (!a)
            h(c, function (a) {
                if (b.options._ddSeriesId === a.lowerSeriesOptions._ddSeriesId)
                    d = a.shapeArgs,
                            d.fill = a.color
    ...
    n.prototype.animateDrillupFrom = function (a) {
        var b = this.chart.options.drilldown.animation, c = this.group, d = this;
    ...
    v && l(v.prototype, {supportsDrilldown: !0, animateDrillupTo: n.prototype.animateDrillupTo, animateDrillupFrom: n.prototype.animateDrillupFrom, animateDrilldown: function (a) {
            var b = this.chart.drilldownLevels[this.chart.drilldownLevels.length - 1], c = this.chart.options.drilldown.animation, d = b.shapeArgs, g = d.start, e = (d.end - g) / this.points.length;
            if (!a)
                h(this.points, function (a, h) {
    ...
    f.Point.prototype.doDrilldown = function (a, b) {
        ...
        if (!c.ddDupes)
            c.ddDupes = [];
        for (; f-- && !e; )
            d.series[f].id === this.drilldown && y(this.drilldown, c.ddDupes) === -1 && (e = d.series[f], c.ddDupes.push(this.drilldown));
        x(c, "drilldown", {point: this, seriesOptions: e, category: b, points: b !== void 0 && this.series.xAxis.ddPoints[b].slice(0)});
    ...
    f.Axis.prototype.drilldownCategory = function (a) {
        ...
        for (b in d)
            (c = d[b]) && c.series && c.series.visible && c.doDrilldown && c.doDrilldown(!0, a);
    ...
    f.Axis.prototype.getDDPoints = function (a, b) {
        ...
        if (!c)
            this.ddPoints = c = {};
        c[a] || (c[a] = []);
        if (c[a].levelNumber !== b)
            c[a].length = 0;
        return c[a]
    };
    w.prototype.drillable = function () {
        var a = this.pos, b = this.label, c = this.axis, d = c.ddPoints && c.ddPoints[a];
        if (b && d && d.length) {
            if (!b.basicStyles)
                b.basicStyles =
                        f.merge(b.styles);
        ...
        } else if (b && b.basicStyles)
            b.styles = {}, b.css(b.basicStyles), b.on("click", null)
    };
    ...
    r(f.Point.prototype, "init", function (a, b, c, d) {
        var g = a.call(this, b, c, d), a = (c = b.xAxis) && c.ticks[d], d = c && c.getDDPoints(d, b.options._levelNumber);
        if (g.drilldown && (f.addEvent(g, "click", function () {
            g.doDrilldown()
        }),
                d))
            d.push(g), d.levelNumber = b.options._levelNumber;
        a && a.drillable();
        return g
    });
    ...
    for (s in p)
        p[s].prototype.supportsDrilldown &&
